...
//        self.datePicker.frame = CGRect(x: self.datePicker.frame.origin.x, y: SCREEN_HEIGHT(), width: self.datePicker.frame.size.width, height: 216)
        self.datePicker.addSubview(JRMonthPicker(frame: CGRect(x: 0, y: self.datePicker.frame.size.height - 216, width: SCREEN_WIDTH(), height: 216), minYear: 2000, maxYear: 2050, delegate: self))
